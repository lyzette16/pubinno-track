...
// Get the desired status from the URL, default to 'forwarded_to_pio'
$requested_status = $_GET['status'] ?? 'forwarded_to_pio';

// Define valid statuses for PIO and their display titles
// Removed 'forwarded_to_external' as a separate tab
$valid_statuses = [
    'forwarded_to_pio'      => 'New Submissions (Forwarded by Facilitators)',
    'accepted_by_pio'       => 'Accepted by PIO',
    'under_external_review' => 'Under External Review', // This tab will now also show 'forwarded_to_external'
    'approved'              => 'Approved (Final)',
    'rejected'              => 'Rejected Submissions'
];

// Validate the requested status, default if invalid
if (!array_key_exists($requested_status, $valid_statuses)) {
    $requested_status = 'forwarded_to_pio';
}
$page_title = $valid_statuses[$requested_status];
...
if (isset($_GET['action'], $_GET['id'])) {
    ...
    if ($oldStmt) {
        ...
        $_SESSION['message_type'] = 'danger';
        header("Location: dashboard.php?status=" . urlencode($requested_status));
        exit();
    }
    ...
    if ($old_status) {
        ...
        switch ($action) {
            ...
            case 'forward_external':
                // Change status to 'under_external_review' as requested
                $new_status = 'under_external_review';
                $notification_message = "Your submission (Ref: **{$reference_number}**) has been **forwarded for external evaluation** by the PIO.";
        ...
        if ($old_status !== $new_status) {
            ...
            try {
                ...
                header("Location: dashboard.php?status=" . urlencode($new_status)); // Redirect to the new status's page
                exit();
...
$submissions = [];
// Join with users table to get researcher's name (u.name)
// JOIN with publication_types to get the specific type name (pt.type_name)
// JOIN with departments to get department name (d.name)
// JOIN with campus to get campus name (c.campus_name)
$stmt = $conn->prepare("
    SELECT
        s.*,
    ...
    WHERE
        (s.status = ? OR (? = 'under_external_review' AND s.status = 'forwarded_to_external')) AND s.campus_id = ?
    ORDER BY
        s.submission_date DESC
");

if ($stmt) {
    // If the requested status is 'under_external_review', also fetch 'forwarded_to_external'
    $status_param = $requested_status;
    $stmt->bind_param("ssi", $status_param, $requested_status, $pio_campus_id);
    $stmt->execute();
    ...
    <div class="container-fluid main-content-wrapper">
        <div id="sidebar">
            ...
            <ul class="nav nav-pills flex-column mb-auto">
                <?php foreach ($valid_statuses as $status_key => $status_title): ?>
                    <li class="nav-item">
                        <a href="dashboard.php?status=<?= urlencode($status_key) ?>" class="nav-link <?= ($requested_status === $status_key) ? 'active' : '' ?>">
            ...
            <ul class="nav nav-pills flex-column mb-auto">
                ...
                <li class="nav-item">
                    <a href="submit_research_pio.php" class="nav-link">
                        <i class="bi bi-upload me-2"></i>
                        Submit Research
                    </a>
                </li>
                <li class="nav-item">
                    <a href="track_submission_pio.php" class="nav-link">
                        ...
                        <tbody>
                            <?php foreach ($submissions as $s): ?>
                                <tr>
                                    ...
                                    <td>
                                        <button type="button" class="btn btn-info btn-sm view-submission-btn" data-bs-toggle="modal" data-bs-target="#submissionDetailsModal" data-submission-id="<?= $s['submission_id'] ?>" data-current-status-filter="<?= urlencode($requested_status) ?>">
                                            View
                                        </button>
                                    </td>
                                </tr>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const setActiveTab = () => {
            ...
            if (status) {
                const tabElement = document.getElementById(status + '-tab');
                if (tabElement) {
        ...
        // Add event listener for tab changes to update URL (optional, for direct linking)
        document.querySelectorAll('a[data-bs-toggle="tab"]').forEach(tabLink => {
            tabLink.addEventListener('shown.bs.tab', function (event) {
                const newStatus = event.target.getAttribute('href').substring(1); // Get tab ID without '#'
        ...
        // Function to confirm action before redirecting
        window.confirmAction = function(actionUrl) {
        ...
        document.querySelectorAll('.view-submission-btn').forEach(button => {
            button.addEventListener('click', function() {
                ...
                const currentStatusFilter = this.dataset.currentStatusFilter || 'forwarded_to_pio'; // Get the status of the tab from which it was opened
                ...
                // The URL assumes get_submission_details_pio.php is in the same directory as dashboard.php
                fetch('get_submission_details_pio.php?submission_id=' + submissionId)
                    ...
                    .then(data => {
                        if (data.success) {
                            ...
                            const baseUrl = `dashboard.php?id=${s.submission_id}&status=${currentStatusFilter}`;

                            if (s.status === 'forwarded_to_pio') {
                                acceptSubmissionBtn.href = `javascript:confirmAction('${baseUrl}&action=accept')`;
                                acceptSubmissionBtn.classList.remove('d-none');

                                rejectSubmissionBtn.href = `javascript:confirmAction('${baseUrl}&action=reject')`;
                                rejectSubmissionBtn.classList.remove('d-none');

                                addCommentBtn.href = `add_comment.php?submission_id=${s.submission_id}&return_page=dashboard&return_status=${currentStatusFilter}`;
                                addCommentBtn.classList.remove('d-none');
                            } else if (s.status === 'accepted_by_pio') {
                                // Removed approve button for 'accepted_by_pio' as requested
                                forwardToExternalBtn.href = `javascript:confirmAction('${baseUrl}&action=forward_external')`;
                                ...
                                addCommentBtn.href = `add_comment.php?submission_id=${s.submission_id}&return_page=dashboard&return_status=${currentStatusFilter}`;
                                addCommentBtn.classList.remove('d-none');
                            } else if (s.status === 'under_external_review') {
                                // PIO can approve/reject from this state
                                // The 'Forward to External' button is intentionally not shown here
                                approveSubmissionBtn.href = `javascript:confirmAction('${baseUrl}&action=approve')`;
