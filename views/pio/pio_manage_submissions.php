...
$valid_statuses = [
    ...
    'forwarded_to_external' => 'Forwarded to External Evaluators',
    'under_external_review' => 'Under External Review', // Assuming this status can exist
...
if (isset($_GET['action'], $_GET['id'])) {
    ...
    if ($oldStmt) {
        ...
        $_SESSION['message_type'] = 'danger';
        header("Location: pio_manage_submissions.php?status=" . urlencode($requested_status));
        exit();
    }
    ...
    if ($old_status) {
        ...
        if ($old_status !== $new_status) {
            ...
            try {
                ...
                header("Location: pio_manage_submissions.php?status=" . urlencode($new_status)); // Redirect to the new status's page
                exit();
        ...
        <div id="sidebar">
            ...
            <ul class="nav nav-pills flex-column mb-auto">
                <?php foreach ($valid_statuses as $status_key => $status_title): ?>
                    <li class="nav-item">
                        <a href="pio_manage_submissions.php?status=<?= urlencode($status_key) ?>" class="nav-link <?= ($requested_status === $status_key) ? 'active' : '' ?>">
                ...
                fetch('get_submission_details_pio.php?submission_id=' + submissionId)
                    ...
                    .then(data => {
                        if (data.success) {
                            ...
                            // All actions redirect back to pio_manage_submissions.php
                            const baseUrl = `pio_manage_submissions.php?id=${s.submission_id}&status=${currentStatusFilter}`;
                            ...
                            if (s.status === 'forwarded_to_pio') {
                                ...
                                addCommentBtn.href = `add_comment.php?submission_id=${s.submission_id}&return_page=pio_manage_submissions&return_status=${currentStatusFilter}`;
