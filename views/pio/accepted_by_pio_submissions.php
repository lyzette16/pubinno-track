...
$requested_status = 'accepted_by_pio';
$page_title = 'Accepted by PIO Submissions';
...
if (isset($_GET['action'], $_GET['id'])) {
    ...
    error_log("PIO Action triggered in accepted_by_pio_submissions.php: " . $action . " for Submission ID: " . $submission_id);
    ...
    if ($old_status) {
        ...
        switch ($action) {
            case 'accept':
                $new_status = 'accepted_by_pio'; // This action might be redundant if already accepted_by_pio
            ...
            case 'forward_external':
                $new_status = 'under_external_review'; // Typo fixed: 'under_extrnal_review' -> 'under_external_review'
                $notification_message = "Your submission (Ref: **" . htmlspecialchars($reference_number ?? 'N/A') . "**) has been **forwarded to external evaluators** by the PIO.";
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        document.querySelectorAll('.view-submission-btn').forEach(button => {
            button.addEventListener('click', function() {
                ...
                approveSubmissionBtn.classList.add('d-none'); // Ensure approve is hidden by default
                    ...
                    .then(data => {
                        if (data.success) {
                            ...
                            if (s.status === 'forwarded_to_pio') {
                                // This page is for 'accepted_by_pio', so 'forwarded_to_pio' status should not appear here for actions,
                                // but if it somehow does, these actions would be relevant.
                                ...
                                // For 'accepted_by_pio', show Forward to External, Approve, Reject, Add Comment
                                ...
                                // For 'under_external_review', show Approve, Reject, Add Comment
                                approveSubmissionBtn.href = `${baseUrl}&action=approve`;
                                approveSubmissionBtn.classList.remove('d-none');
                                ...
                                // If already forwarded to external, PIO can still approve/reject or add comments
                                approveSubmissionBtn.href = `${baseUrl}&action=approve`;
                                approveSubmissionBtn.classList.remove('d-none');
