...
$requested_status = 'under_external_review';
$page_title = 'Submissions Under External Review';

// Handle PIO actions: approve / reject (these actions are typically done from this page, but not via buttons in modal)
// The logic below still exists in case a direct URL action is attempted, but the UI buttons are removed.
if (isset($_GET['action'], $_GET['id'])) {
    $submission_id = (int) $_GET['id'];
    $action = $_GET['action'];

    error_log("PIO Action triggered in under_external_review_submissions.php: " . $action . " for Submission ID: " . $submission_id);
    ...
    if ($oldStmt) {
        ...
        $_SESSION['message_type'] = 'danger';
        header("Location: under_external_review_submissions.php"); // Redirect to this specific page
        exit();
    }
    ...
    if ($old_status) {
        ...
        switch ($action) {
            case 'approve':
                $new_status = 'approved';
                $notification_message = "Your submission (Ref: **" . htmlspecialchars($reference_number ?? 'N/A') . "**) has been **approved** by the PIO.";
            ...
            case 'reject':
                ...
                break;
            default:
                $_SESSION['message'] = 'Invalid action requested for this status.';
                $_SESSION['message_type'] = 'warning';
                header("Location: under_external_review_submissions.php"); // Redirect to this specific page
                exit();
        }

        // Only update if the status is actually changing and the action is valid for this status
        if ($old_status !== $new_status && ($action === 'approve' || $action === 'reject')) {
            ...
            try {
                ...
                header("Location: under_external_review_submissions.php"); // Redirect back to this specific page
                exit();
            ...
            // Status did not actually change or invalid action for this status
            $_SESSION['message'] = 'Submission status is already ' . htmlspecialchars(str_replace('_', ' ', $old_status)) . ' or action is not permitted for this status. No change made.';
        ...
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    ...
                    <!-- Only Add Comment button remains as per request -->
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Removed approveSubmissionBtn and rejectSubmissionBtn from JS constants
        ...
        document.querySelectorAll('.view-submission-btn').forEach(button => {
            button.addEventListener('click', function() {
                ...
                addCommentBtn.classList.add('d-none'); // Only addCommentBtn remains
                ...
                fetch('get_submission_details_pio.php?submission_id=' + submissionId)
                    ...
                    .then(data => {
                        if (data.success) {
                            ...
                            // For 'under_external_review' status, only show 'Add Comment'
                            if (s.status === 'under_external_review') {
                                ...
                                // For any other status, if you want to allow comments, keep this.
                                // Otherwise, if no actions are needed for other statuses on this page,
                                // you can remove this else block entirely.
                                addCommentBtn.href = `add_comment_pio.php?submission_id=${s.submission_id}&return_page=${currentFileName}`;
                                addCommentBtn.classList.remove('d-none');
                            }
