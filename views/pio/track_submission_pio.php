<?php
// File: views/pio/track_submission.php
...
$pio_campus_id = $_SESSION['campus_id'] ?? null; // Get PIO's campus ID
...
if (!$pio_campus_id) {
    $track_message = '<div class="alert alert-danger">Your PIO account is not associated with a campus. Cannot track submissions.</div>';
}

// Define the steps in the tracking process (removed 'forwarded_to_external')
$statusSteps = [
    'submitted',
...
if ($submission_identifier && $pio_campus_id) {
    // Modified SQL query to explicitly select the rejector's name from the `submissions` table.
    // This is useful if the `rejected_by` column is consistently updated by all roles.
    ...
    if ($identifier_type === 'id') {
        $stmt = $conn->prepare($base_sql . " WHERE s.submission_id = ? AND s.campus_id = ?");
        $stmt->bind_param("ii", $submission_identifier, $pio_campus_id);
    } else { // identifier_type === 'reference'
        $stmt = $conn->prepare($base_sql . " WHERE s.reference_number = ? AND s.campus_id = ?");
        $stmt->bind_param("si", $submission_identifier, $pio_campus_id);
    }
    ...
    if ($result->num_rows > 0) {
        ...
        $stmt2 = $conn->prepare("SELECT new_status, changed_at, u.name AS changed_by_name FROM submission_status_logs l JOIN users u ON l.changed_by = u.user_id WHERE submission_id = ? ORDER BY changed_at ASC");
        ...
        foreach ($logs as $log) {
            ...
            if ($mapped) {
                if (!isset($stepTimestamps[$mapped])) {
                   ...
                   $stepTimestamps[$mapped]['by'] = htmlspecialchars($log['changed_by_name']);
                }
        ...
        $track_message = '<div class="alert alert-danger">No submission found with that reference number for your campus.</div>';
    }
    $stmt->close();
...
<html>
<head>
    <title>Track Submission (PIO)</title>
    ...
    <style>
        ...
        .navbar {
            background-color: #e9f0f8; /* Light blue for PIO */
...
<body class="bg-light">
    ...
    <div class="container-fluid main-content-wrapper">
        ...
        <div id="main-content">
            <h4 class="mb-4">Track Submission (All Departments)</h4>
            ...
            <form method="POST" class="bg-white p-4 shadow rounded mb-4">
                <div class="mb-3">
                    ...
                    <input type="text" name="reference_number" id="reference_number" class="form-control" required placeholder="e.g., SUB-1234567890-abcdefgh" value="<?= htmlspecialchars($_POST['reference_number'] ?? $_GET['ref'] ?? '') ?>">
                </div>
            ...
            <?php if ($submission): ?>
                <div class="bg-white p-4 shadow rounded mb-4">
                    ...
                    <h6 class="mb-3" >Title: <?= htmlspecialchars($submission['title']) ?> <br> <hr>Researcher: <?= htmlspecialchars($submission['researcher_name']) ?><br><br> Department: <?= htmlspecialchars($submission['department_name'] ?? 'N/A') ?></h6>
